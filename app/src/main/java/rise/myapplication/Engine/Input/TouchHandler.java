...
public class TouchHandler implements View.OnTouchListener {
     ...
     * events.
     */
    private final boolean[] mExistsTouch = new boolean[MAX_TOUCHPOINTS];
    private final float[] mTouchX = new float[MAX_TOUCHPOINTS];
    private final float[] mTouchY = new float[MAX_TOUCHPOINTS];

    /**
     * Touch event pool and lists of current (for this frame) and unconsumed
     * (occurring since the frame started) touch events.
     */
    private final Pool<TouchEvent> mPool;
    private final List<TouchEvent> mTouchEvents = new ArrayList<>();
    private final List<TouchEvent> mUnconsumedTouchEvents = new ArrayList<>();
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        synchronized (this) {
            if(touchEvent.type == TouchEvent.TOUCH_DOWN || touchEvent.type == TouchEvent.TOUCH_UP)
